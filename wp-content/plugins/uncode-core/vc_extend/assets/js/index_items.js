	...
	window.itemIndex = function() {
		...
		function load_item($query) {
			...
			$.each($templateCatEl, function(index, val) {
				...
				switch (String($type[0])) {
					case ('post'):
						$templatePost.append($(this).clone());
						break;
					case ('portfolio'):
						$templatePortfolio.append($(this).clone());
						break;
					case ('page'):
						$templatePage.append($(this).clone());
						break;
					case ('product'):
						$templateProduct.append($(this).clone());
						break;
				}
			});
			/** reset sort list **/
			$('.vc_sorted-list-container', $templatePost).html('');
			$('.vc_sorted-list-container', $templatePortfolio).html('');
			$('.vc_sorted-list-container', $templatePage).html('');
			$('.vc_sorted-list-container', $templateProduct).html('');
			/** load sort list **/
			$.ajax({
				type: 'POST',
				url: SiteParameters.admin_ajax,
				data: {
					action: 'uncode_get_query',
					content: $query,
					allItems: $allItems,
					templateSingle: $templateSingle.html(),
					templatePost: $templatePost.html(),
					templatePortfolio: $templatePortfolio.html(),
					templatePage: $templatePage.html(),
					templateProduct: $templateProduct.html(),
					custom_order: $custom_order,
					order_ids: $order_ids,
					postid: $bundleInput.attr('data-post')
				},
				dataType: 'html',
			}).done(function(html) {
				init_single_item_list(html);
